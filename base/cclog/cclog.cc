...
class Logger {
  ...
  protected:
    safe::Mutex _log_mtx;
#if __cplusplus >= 201103L
	std::unique_ptr<safe::StoppableThread> _log_thread;
#else
...
Logger::Logger(uint32 ms)
    ...
    _log_thread.reset(
#if __cplusplus >= 201103L
	new safe::StoppableThread(std::bind(&Logger::thread_fun, this), _ms));//创建一个定时运行且可停止的线程
#else
...
void Logger::thread_fun() {
    {
        ...
        if (!_logs.empty()) _logs.swap(_temp);//交换的速度是常数级别的，因为swap底层只是交换两个容器的内部数据结构，并不会交换数据
    }
...
bool TaggedLogger::open_log_file(const char* tag) {
    std::string time =
        sys::local_time.to_string(_strategy[tag] == 0 ? "%Y%m%d" : "%Y%m%d%H");//默认按日存储

    std::string name = _log_prefix + kProgName + "." + time + "." + tag;//构造文件名
    std::string path = _log_dir + name;//加上路径

    if (!os::path.exists(_log_dir)) os::makedirs(_log_dir);//不存在目录新建一个
    if (!_files[tag].open(path, "a")) return false;//追加方式打开文件

    std::string link_path = _log_dir + kProgName + "." + tag;//链接名
...
inline void TaggedLogger::log_to_file(const std::string& time, TaggedLog* log) {
    ...
    if (!file && !this->open_log_file(log->type())) return;//如果打开该文件失败

    file.write(time);
    file.write(log->data(), log->size());
}

//实现继承自基类的接口（TLOG("tag") ==> log to file xx_tag_xx.log）
...
void TaggedLogger::flush_log_files() {
    ...
    for (std::map<const char*, os::file>::iterator 
        ...
        if (file.exists()) file.flush();//将数据刷入磁盘空间
        if (file.exists() && day == _last_day &&
            (hour == _last_hour || _strategy[it->first] != by_hour)) {
            continue;
        }

        file.close();//如果过了设置的间隔时间段则关闭以便下次重新生成新文件
    }
}
