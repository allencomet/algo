...
struct local_time {
    ...
    static inline std::string year(){
		return local_time::to_string(local_time::sec(), "%Y");
	}

	static inline std::string month(){
		return local_time::to_string(local_time::sec(), "%m");
	}

    static inline int32 minute_in_day(){
		std::vector<std::string> v = str::split(local_time::to_string(local_time::sec(), "%H:%M"),':');
		switch(v.size()){
		case 1:
			return  str::to_int32(v[0])*60;
			break;
		case 2:
			return  str::to_int32(v[0])*60 + str::to_int32(v[1]);
			break;
        default:
            return 0;
            break;
...
class signal_handler {
  public:
    static signal_handler* instance() {
        static signal_handler sh;//�ֲ���̬����ֵֻ����һ��
        return &sh;
    }
  ...
  private:
    std::map<int, cbque> _map;//ÿ���źŶ�Ӧ�Ķ����źŻص�ӳ��
...
inline void signal::add_handler(int sig, boost::function<void()> cb, int flag) {
	signal_handler::instance()->add_handler(sig, cb, flag);//ע���źŶ�Ӧ�ص�
}
